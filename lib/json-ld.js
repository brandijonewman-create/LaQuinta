...
export function organizationSchema() {
  return {
    ...
    // Parent / operating entity. 7671 Enterprises LLC is the legal owner.
    parentOrganization: { '@type': 'Organization', name: owner.name },
...
export function personSchema(person) {
  ...
  // Schema.org expects absolute URLs for `image` — promote relative paths.
  const image = person.headshot
    ? (person.headshot.startsWith('http') ? person.headshot : `${site.url}${person.headshot}`)
    : undefined;
  return {
    '@context': 'https://schema.org',
    '@type': 'Person',
  ...
  const articleAuthor = author
    ? {
        ...
        image: author.headshot
          ? (author.headshot.startsWith('http') ? author.headshot : `${site.url}${author.headshot}`)
          : undefined,
