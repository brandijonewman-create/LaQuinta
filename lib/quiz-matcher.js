...
export const communityAttrs = {
  ...
  'pga-west': {
    name: 'PGA West',
    city: 'La Quinta',
    setting: 'resort-scale',
    era: 'bold-visual',
    character: 'tour-adjacent',
    footprint: 'mixed',
    fit: ['snowbird', 'primary', 'investor'],
    why: 'Six courses, four architects, the widest price band and most flexible membership tiers in the city.',
  },
...
export function scoreQuiz(answers) {
  const results = Object.entries(communityAttrs).map(([slug, c]) => {
    let score = 0;
    const reasons = [];
